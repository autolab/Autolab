...
function KeyTable ( oInit )
{
	...
	function _fnKey ( e )
	{
        ...
        if ( oInit.num < 0) {
            oInit.num = iTableWidth - oInit.start
        }
		...
		switch( iKey )
		{
			...
			case 37: /* left arrow */
                // AUTOLAB
				if ( _iOldX > oInit.start) {
					x = _iOldX - 1;
					y = _iOldY;
				} else if ( _iOldY > 0 ) {
					//x = iTableWidth-1;
                    x = oInit.start + oInit.num - 1
					y = _iOldY - 1;
				} else {
					/* at start of table */
					if ( iKey == -1 && _bForm )
			...
			case 39: /* right arrow */
                // AUTOLAB
                if ( _iOldX < oInit.start + oInit.num - 1) {
	...
	function _fnCellFromCoords( x, y )
	{
		if ( _oDatatable )
		{
			...
			{
                // AUTOLAB
				return jQuery(oSettings.aoData[ oSettings.aiDisplay[ y ] ].nTr).children('td')[x];
	...
	function _fnFindDtCell( nTarget )
	{
		...
		for ( var i=0, iLen=oSettings.aiDisplay.length ; i<iLen ; i++ )
		{
			...
			var nTds = jQuery(nTr).children('td');
	...
	function _fnInit( oInit, that )
	{
		...
		if ( typeof oInit.start == 'undefined' ) {
			oInit.start = 0;
		}

		if ( typeof oInit.num == 'undefined' ) {
			oInit.num = -1;
		}
		...
		{
            // AUTOLAB
			jQuery('tbody td.edit', _oDatatable.fnSettings().nTable).live( 'click', _fnClick );
		}
		else
		{
