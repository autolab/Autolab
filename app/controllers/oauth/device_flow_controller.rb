...
module Oauth
  class DeviceFlowController < ActionController::Base
    ...
    def init
      ...
      if req.nil?
        raise DeviceFlowError.new("Failed to create request, try again later",
                                  :internal_server_error)
      end

      # success!
      render json: { device_code: req.device_code, user_code: req.user_code,
                     verification_uri: device_flow_activation_url }.to_json
    end

    # expects params: client_id, device_code
    def authorize
      unless params.key?(:device_code)
      ...
      # check if authorized
      raise DeviceFlowError.new("authorization_pending", :bad_request) unless req.is_resolved

      unless req.is_granted
  ...
  private
    ...
    def set_app
      ...
      raise DeviceFlowError.new("Unrecongized client_id", :bad_request) if @app.nil?
    end
  end
end
