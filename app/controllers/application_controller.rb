...
protected
  ...
  def set_course
    ...
    render(file: "#{Rails.root}/public/404.html", status: :not_found) && return unless @course
  ...
  def run_scheduler
    actions = Scheduler.where("next < ?", Time.zone.now)
    actions.each do |action|
      action.next = Time.zone.now + action.interval
      action.save
