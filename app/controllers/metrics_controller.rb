...
class MetricsController < ApplicationController
  ...
  def get_watchlist_instances
    ...
    user_hash = user_data.each_with_object({}) do |entry, hash|
    ...
    risk_hash = risk_condition_data.each_with_object({}) do |entry, hash|
      hash[entry["id"]] = entry
    end
  ...
  def new_metrics_params
    if params[:metric].present?
      params.require(:metric).permit(grace_day_usage: %i[grace_day_threshold date],
                                     grade_drop: %i[percentage_drop
                                                    consecutive_counts],
                                     no_submissions: [:no_submissions_threshold],
                                     low_grades: %i[grade_threshold
                                                    count_threshold])
    end
  end
end
