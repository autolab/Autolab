...
module Api
  module V1
    class CoursesController < Api::V1::BaseApiController
      ...
      def index
        ...
        if params.key?(:state)
          if params[:state] == "disabled"
            courses_for_user = courses_for_user.select(&:disabled?)
          elsif %w[completed current upcoming].include? params[:state]
        ...
        courses_for_user.each do |course|
          course_hash = course.as_json(only: %i[name display_name semester late_slack
                                                grace_days auth_level])
      ...
      def create
        require_params(%i[name semester instructor_email])

        begin
          # name validation done during create
          newCourse = Course.quick_create(params[:name], params[:semester],
        ...
        newCourse.start_date = safe_parse_date(params[:start_date]) if params.key?(:start_date)
        newCourse.end_date = safe_parse_date(params[:end_date]) if params.key?(:end_date)

        # attempt save
        unless newCourse.save
          raise ApiError.new("Failed to create course: #{validation_errors_for(newCourse)}",
                             :bad_request)
        end
