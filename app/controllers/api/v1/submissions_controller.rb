...
module Api
  module V1
    class SubmissionsController < Api::V1::BaseApiController
      ...
      def feedback
        ...
        unless problem
          raise ApiError.new("Problem named #{params[:problem]} does not exist for #{@assessment.name}",
                             :not_found)
        end
        ...
        unless submission
          raise ApiError.new("Submission version #{vers} does not exist for #{@assessment.name}",
                             :not_found)
        end
        ...
        unless score
          raise ApiError.new(
            "Score for #{params[:problem]} of submission version #{vers} of #{@assessment.name} does not exist", :not_found
          )
        end
