...
module Api
  module V1
    class CourseUserDataController < Api::V1::BaseApiController
      ...
      def create
        require_params(%i[email lecture section auth_level])
      ...
      def update
        ...
        unless cud.save && cud.update(update_cud_params)
      ...
      def destroy
        ...
        unless cud.update(dropped: true)
          raise ApiError.new("Update failed: #{validation_errors_for(cud)}", :bad_request)
        end
    ...
    private
      ...
      def set_user
        ...
        raise ApiError.new("Nonexistent user", :bad_request) if @user.nil?
      end

      def get_user_cud
        cud = @user.course_user_data.find_by(course: @course)
        raise ApiError.new("User is not in course", :not_found) if cud.nil?

        cud
      end

      def format_cud_response(cud)
        cud_hash = cud.as_json(only: %i[lecture section grade_policy nickname dropped])
        user_hash = cud.user.as_json(only: %i[first_name last_name email school major year])
