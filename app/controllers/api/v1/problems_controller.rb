class Api::V1::ProblemsController < Api::V1::BaseApiController
  ...
  def index
    ...
    respond_with problems, only: [:name, :description, :max_score, :optional, :favorite]
  end

  # endpoint for creating a problem for an assessment
  def create
    require_params([:name, :max_score])
    set_default_params({ description: "", optional: false, favorite: false })
...
private
  ...
  def problem_params
    params.permit(:name, :description, :max_score, :optional, :favorite)
  end

  def format_problem_response(problem)
    problem.as_json(only: [:name, :description, :max_score, :optional, :favorite])
  end
end
