  ...
  module V1
    class AssessmentsController < Api::V1::BaseApiController
      ...
      before_action lambda {
                    ...
                    }, only: %i[index problems writeup handout]
      ...
      def index
        ...
        allowed = %i[name display_name start_at due_at end_at category_name]
        if @cud.student?
      ...
      def show
        allowed = %i[name display_name description start_at due_at end_at updated_at max_grace_days max_submissions
        ...
        allowed += [:grading_deadline] unless @cud.student?
        ...
        if @assessment.overwrites_method?(:handout) || @assessment.handout_is_file?
      ...
      def problems
        ...
        respond_with problems, only: %i[name description max_score optional]
      end

      # endpoint for obtaining the writeup
      def writeup
        respond_with_hash({ url: @assessment.writeup }) and return if @assessment.writeup_is_url?
      ...
      def handout
        ...
        respond_with_hash({ url: @assessment.handout }) and return if @assessment.handout_is_url?
