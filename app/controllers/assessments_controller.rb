  ...
  def install_assessment
    ...
    Dir.foreach(ass_dir) do |filename|
      ...
      if filename =~ /[^a-z0-9]/
        ...
        flash.now[:error] = flash[:error] ? "#{flash[:error]} <br>" : ""
        flash.now[:error] += "An error occured while trying to display an existing assessment on " \
            "file directory #{filename}: assessment file names must only contain lowercase " \
      ...
      unless File.exist?(File.join(ass_dir, filename, "#{filename}.yml"))
        flash.now[:error] = flash[:error] ? "#{flash[:error]} <br>" : ""
        flash.now[:error] += "An error occured while trying to display an existing assessment on " \
          "file directory #{filename}: #{filename}.yml does not exist"
        flash.now[:html_safe] = true
        next
      end
