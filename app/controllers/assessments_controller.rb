...
class AssessmentsController < ApplicationController
  ...
  def index
    ...
    @announcements = Announcement.where("start_date < :now AND end_date > :now", now: Time.zone.now)
  ...
  def installAssessment
    ...
    Dir.foreach(ass_dir) do |filename|
      ...
      if !File.directory?(File.join(ass_dir, filename)) || (filename == "..") || (filename == ".")
        next
      end
  ...
  def create
    ...
    @assessment.visible_at = Time.zone.now
    @assessment.start_at = Time.zone.now
    @assessment.due_at = Time.zone.now
    @assessment.end_at = Time.zone.now
    @assessment.grading_deadline = Time.zone.now
  ...
  def export
    ...
    begin
      ...
      tarStream.rewind
      tarStream.close
      send_data tarStream.string.force_encoding("binary"), filename: "#{@assessment.name}_#{Time.zone.now.strftime('%Y%m%d')}.tar", content_type: "application/x-tar"
  ...
  def releaseAllGrades
    ...
    if num_released > 0
      flash[:success] = format("%d %s released.", num_released, (num_released > 1 ? "grades were" : "grade was"))
    else
