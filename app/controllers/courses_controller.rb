...
file, most likely a duplicate email.  The exact error was: #{e} "
  ...
  def email
    if request.post?
      section = (params[:section] if params[:section].length.positive?)
  ...
  def runMoss
    ...
    @mossExit = $CHILD_STATUS.exitstatus
...
private
  ...
  def parse_roster_csv
    ...
    begin
      ...
      csv.each do |row|
        next if row[1].nil? || row[1].chomp.size.zero?
