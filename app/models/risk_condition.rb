...
class RiskCondition < ApplicationRecord
  ...
  enum condition_type: %i[no_condition_selected grace_day_usage grade_drop no_submissions
                          low_grades]
  ...
  def self.update_current_for_course(course_name, params)
    ...
    max_version = RiskCondition.get_max_version(course_id)
    # Is params empty?
    if params.length.zero? && max_version.zero?
    ...
    if params.length.zero?
      if previous_types.length.positive? && (previous_types.none? do |t|
                                               t == "no_condition_selected"
                                             end)
  ...
  def self.get_grade_drop_condition_for_course(course_name)
    ...
    return nil if current_conditions.count.zero?
