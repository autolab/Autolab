...
class Course < ApplicationRecord
  ...
  def self.quick_create(unique_name, semester, instructor_email)
    ...
    # Ensure app user (9999) has access before we lock down the directory
    newCourse.ensure_service_user_group_membership!
  ...
  def ensure_default_group_instructor!
    ...
    unless default_user
      Rails.logger.warn("Default group instructor '#{identifier}' not found; " \
                          "skipping auto-enrollment for course #{name}")
      return
    end
    ...
    unless cud.save
      Rails.logger.warn("Failed to enroll default instructor #{default_user.email} " \
                          "for course #{name}: " \
                          "#{cud.errors.full_messages.join(', ')}")
    end
  rescue StandardError => e
    Rails.logger.warn("Failed to ensure default instructor for course #{name}: " \
                        "#{e.class} - #{e.message}")
  end
  ...
  def init_course_folder
    ...
    begin
      ...
      Rails.logger.info("Course directory #{dir_path} created with ownership: " \
    ...
    # Log file creation
    Rails.logger.info("Course files created: autolab.log=#{autolab_log_path}, " \
                        "course.rb=#{course_rb_path}")
    begin
      ...
      if log_stat
        Rails.logger.info("File ownership after creation - autolab.log: " \
                            "uid=#{log_stat.uid}, gid=#{log_stat.gid}, " \
                            "mode=#{log_stat.mode.to_s(8)}")
      end

      if rb_stat
        Rails.logger.info("File ownership after creation - course.rb: " \
                            "uid=#{rb_stat.uid}, gid=#{rb_stat.gid}, mode=#{rb_stat.mode.to_s(8)}")
      end
    rescue StandardError => e
    ...
    if group_name
      if UnixGroupManager.delegate_enabled?
        ...
        Rails.logger.info("Setting group ownership on directory and files via " \
                            "delegate (keeping Rails process as owner)")
        ...
        if chgrp_success
          ...
          begin
            ...
            Rails.logger.info("Directory before chmod: uid=#{stat_before_chmod.uid}, " \
                                "gid=#{stat_before_chmod.gid}, " \
                                "mode=#{stat_before_chmod.mode.to_s(8)}")
          rescue StandardError => e
          ...
          begin
            ...
            Rails.logger.info("Directory after chmod: uid=#{stat_after_chmod.uid}, " \
                                "gid=#{stat_after_chmod.gid}, " \
                                "mode=#{stat_after_chmod.mode.to_s(8)}")
          rescue StandardError => e
          ...
          if File.exist?(autolab_log_path)
            file_chgrp_success = UnixGroupManager.chgrp_path(autolab_log_path, group_name,
                                                             owner: nil)
            Rails.logger.info("autolab.log chgrp result: " \
                                "#{file_chgrp_success ? 'success' : 'failed'}")
            begin
              log_stat = File.stat(autolab_log_path)
              Rails.logger.info("autolab.log after chgrp: uid=#{log_stat.uid}, " \
                                  "gid=#{log_stat.gid}, mode=#{log_stat.mode.to_s(8)}")
            rescue StandardError => e
          ...
          if File.exist?(course_rb_path)
            ...
            Rails.logger.info("course.rb chgrp result: " \
                                "#{file_chgrp_success ? 'success' : 'failed'}")
            begin
              rb_stat = File.stat(course_rb_path)
              Rails.logger.info("course.rb after chgrp: uid=#{rb_stat.uid}, " \
                                  "gid=#{rb_stat.gid}, mode=#{rb_stat.mode.to_s(8)}")

              # Test if file is readable
              if File.readable?(course_rb_path)
                Rails.logger.info("course.rb is readable by current process")
              else
                Rails.logger.warn("course.rb is NOT readable by current process - " \
                                  "this will cause reload_config_file to fail!")
              end
            rescue StandardError => e
          ...
          begin
            ...
            if gid && stat_after.gid == gid
              Rails.logger.info("Successfully set group #{group_name} (gid #{gid}) on course " \
                                  "directory #{dir_path} via delegate " \
                                  "(owner remains Rails process for now)")
            else
              Rails.logger.warn("Group ownership mismatch - expected gid #{gid}, " \
                                  "got #{stat_after.gid}")
            end
            ...
            if File.readable?(dir_path) && File.executable?(dir_path)
              ...
              Rails.logger.warn("Directory is NOT readable/executable by current process - " \
                                  "this will cause reload_config_file to fail!")
            end
          rescue StandardError => e
          ...
          Rails.logger.error("Failed to set group #{group_name} via delegate chgrp. " \
                               "Ensure UnixOps daemon is running and accessible.")
        end
      else
        ...
        begin
          ...
          Rails.logger.info("Successfully set group #{group_name} (gid #{gid}) on " \
                              "course directory #{dir_path}")
        rescue ArgumentError
          Rails.logger.error("Group #{group_name} not found locally. Delegate is not " \
                               "enabled, so local lookup is required.")
        end
      end
    end
    ...
    if File.exist?(autolab_log_path)
      ...
      FilesystemEnforcer.fix_path(autolab_log_path, group_name:)
      begin
        log_stat = File.stat(autolab_log_path)
        Rails.logger.info("autolab.log after FilesystemEnforcer: uid=#{log_stat.uid}, " \
                            "gid=#{log_stat.gid}, mode=#{log_stat.mode.to_s(8)}")
      rescue StandardError => e
    ...
    if File.exist?(course_rb_path)
      ...
      FilesystemEnforcer.fix_path(course_rb_path, group_name:)
      begin
        rb_stat = File.stat(course_rb_path)
        path = course_rb_path
        Rails.logger.info("course.rb after FilesystemEnforcer: uid=#{rb_stat.uid}, " \
                            "gid=#{rb_stat.gid}, mode=#{rb_stat.mode.to_s(8)}")
        Rails.logger.info("course.rb readable? #{File.readable?(path)}, " \
                            "executable? #{File.executable?(path)}")
      rescue StandardError => e
        Rails.logger.warn("Could not stat course.rb after FilesystemEnforcer: " \
                            "#{e.message}")
      end
    else
    ...
    # Note: We intentionally do NOT set strict directory permissions (drwxrws---) here because
    # the Rails process needs to access the directory to read course.rb during reload_course_config
    ...
    Rails.logger.info("Ensuring courseConfig and assessmentConfig are writable " \
                        "via delegate (Rails uid=#{rails_uid}, group=#{autolab_group})")
    ...
    [course_config_dir, assessment_config_dir].each do |dir|
      ...
      Rails.logger.info("Set #{dir} via delegate: owner=#{rails_uid}, " \
                          "group=#{autolab_group}, mode=775")
    end
  ...
  def reload_config_file
    ...
    Rails.logger.info("Current process: uid=#{Process.uid}, gid=#{Process.gid}, " \
    ...
    begin
      ...
      Rails.logger.info("Source file stats: uid=#{stat.uid}, gid=#{stat.gid}, " \
      ...
      dir_path = File.dirname(source_config_file_path)
      Rails.logger.info("Source directory stats: uid=#{dir_stat.uid}, " \
                          "gid=#{dir_stat.gid}, mode=#{dir_stat.mode.to_s(8)}")
      Rails.logger.info("Source directory readable? #{File.readable?(dir_path)}")
      Rails.logger.info("Source directory executable? #{File.executable?(dir_path)}")
    rescue StandardError => e
    ...
    begin
      ...
      if File.readable?(source_config_file_path)
        ...
        raise "Cannot read #{source_config_file_path} - " \
                "permission denied and delegate not available"
      end
    rescue Errno::EACCES => e
      error_msg = "Permission denied reading #{source_config_file_path}: #{e.message}. " \
        "Check file permissions and directory permissions."
      Rails.logger.error(error_msg)
      Rails.logger.error("File stats: #{begin
        File.stat(source_config_file_path).inspect
      rescue StandardError
        'could not stat'
      end}")
      raise error_msg
    ...
    begin
      ...
      if config_dir_stat
        Rails.logger.info("courseConfig directory stats: uid=#{config_dir_stat.uid}, " \
                            "gid=#{config_dir_stat.gid}, mode=#{config_dir_stat.mode.to_s(8)}")
      end
    ...
    begin
      ...
      error_msg = "Permission denied writing to #{config_file_path}: #{e.message}. " \
        "Check courseConfig directory permissions."
      Rails.logger.error(error_msg)
      raise error_msg
    rescue StandardError => e
    ...
    begin
      ...
      Rails.logger.info("eval(#{config_module_name}) returned: " \
                          "#{eval_result.class} - #{eval_result.inspect[0..100]}")
    rescue StandardError => e
    ...
    if eval_result.is_a?(Module)
      ...
      Rails.logger.info("eval returned #{eval_result.class}, " \
                          "trying Object.const_get(#{config_module_name})")
      begin
        mod = Object.const_get(config_module_name)
        Rails.logger.info("Object.const_get succeeded, got module: #{mod.class}")
      rescue NameError => e
        Rails.logger.error("Object.const_get(#{config_module_name}) failed: " \
                             "#{e.class} - #{e.message}")
    ...
    Rails.logger.info("=== Successfully completed reload_config_file for course: #{name}, " \
                        "returning module: #{mod.class} (#{mod}) ===")
    mod
  end
  ...
  def reload_course_config
    ...
    Rails.logger.info("This will call reload_config_file, then extend " \
                        "AdminsController with the module")
    begin
      ...
      Rails.logger.info("Module details: name=#{begin
        mod.name
      rescue StandardError
        'no name'
      end}, class=#{mod.class}, instance_of?(Module)=#{mod.is_a?(Module)}")

      unless mod.is_a?(Module)
        error_msg = "reload_config_file did not return a Module, " \
          "got: #{mod.class} - #{mod.inspect[0..200]}"
        Rails.logger.error(error_msg)
        raise TypeError, error_msg
      end
...
private
  ...
  private_class_method :default_group_instructor_identifier
  ...
  private_class_method :default_group_instructor_user
  ...
  def ensure_unix_group_exists
    ...
    if success
      ...
      Rails.logger.error("Failed to create Unix group #{group_name} via delegate " \
                           "for course #{name}")
      delegate_url = ENV['UNIX_OPS_DELEGATE_URL']
      Rails.logger.error("Check that UnixOps daemon is running and accessible at #{delegate_url}")
    end
  end
  ...
  def ensure_service_user_group_membership!
    ...
    # 1. Ensure the group exists on the AWS Host (via Daemon)
    unless UnixGroupManager.ensure_group(group_name)
      Rails.logger.error("Course #{name}: cannot ensure group #{group_name} exists on host")
      return false
    end

    # 2. Add user(s) to group inside the host and local container
    gid = UnixGroupManager.get_group_gid(group_name)
    per_user_results = sync_users.map do |username|
      remote_success = UnixGroupManager.add_user_to_group(username, group_name)
      local_success = UnixGroupManager.ensure_local_group_membership(username,
                                                                     group_name, gid_hint: gid)
      { username:, remote_success:, local_success: }
    end

    all_success = per_user_results.all? { |result|
      result[:remote_success] && result[:local_success]
    }

    if all_success
      Rails.logger.info("Course #{name}: dual-synced users " \
                          "#{sync_users.join(', ')} in group #{group_name}")
      true
    else
      failed = per_user_results.reject { |result|
        result[:remote_success] && result[:local_success]
      }
      Rails.logger.warn("Course #{name}: failed to fully sync group membership for " \
                          "#{failed.map { |r|
                               "#{r[:username]}(remote=#{r[:remote_success]}," \
                                                         "local=#{r[:local_success]})"
                             }.join(', ')}")
      false
    end
  end
