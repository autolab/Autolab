...
class Submission < ApplicationRecord
  ...
  def save_file(upload)
    ...
    if upload["file"]
      # Sanity!
      upload["file"].rewind
      File.open(create_and_return_handin_file_path, "wb") { |f| f.write(upload["file"].read) }
      ...
      File.open(create_and_return_handin_file_path, "wb") do |f|
      ...
      FileUtils.mv(src, create_and_return_handin_file_path)
    end
  ...
  def handin_file_path
    ...
    new_handin_file_path = File.join(assessment.handin_directory_path, course_user_datum.email, filename)
    unless File.exist?(new_handin_file_path)
      return File.join(assessment.handin_directory_path, filename)
    end
    new_handin_file_path
  end
  def create_and_return_handin_file_path
    return nil unless filename
  ...
  def handin_annotated_file_path
    ...
    new_handin_annotated_file_path = File.join(assessment.handin_directory_path, course_user_datum.email, "annotated_#{filename}")
    unless File.exist?(new_handin_annotated_file_path)
      return File.join(assessment.handin_directory_path, "annotated_#{filename}")
    end
    new_handin_annotated_file_path
  end
  ...
  def autograde_feedback_path
    ...
    new_autograde_feedback_path = File.join(assessment.handin_directory_path, course_user_datum.email,
              autograde_feedback_filename)
    unless File.exist?(new_autograde_feedback_path)
      return File.join(assessment.handin_directory_path, old_autograde_feedback_filename)
    end
