...
class WatchlistInstance < ApplicationRecord
  enum status: %i[pending contacted resolved], _suffix: "watchlist"
  ...
  def self.refresh_instances_for_course(course_name, metrics_update = false)
    ...
    deprecated_instances.each(&:archive_watchlist_instance) if metrics_update
  ...
  def self.update_course_gdu_watchlist_instances(course)
    ...
    if asmts_before_date.count.zero?
    ...
    unless parameters_grade_drop.nil?
      ...
      course_user_data.each do |cud|
        new_instance = add_new_instance_for_cud_grade_drop(course, grade_drop_condition_id,
                                                           cud, asmt_arrs, consecutive_counts, percentage_drop)
        new_instances << new_instance unless new_instance.nil?
      end
    end
  ...
  def archive_watchlist_instance
    if pending_watchlist?
      ...
      self.archived = true
      unless save
        raise "Failed to archive watchlist instance for user #{course_user_datum.user_id} in course #{course.display_name}"
      end
    end
  end
    ...
    course_user_data.each do |cud|
      ...
      conditions.each do |condition|
        ...
        when "grace_day_usage"
          ...
          asmts_before_date = course.asmts_before_date(date)
          next if asmts_before_date.count.zero?
        ...
        when "grade_drop"
          ...
          new_instance = add_new_instance_for_cud_grade_drop(course, condition.id, cud,
                                                             asmt_arrs, consecutive_counts, percentage_drop)
          cur_instances << new_instance unless new_instance.nil?
      ...
      cur_instances.each do |inst|
        ...
        all_dup = false if cr_dup.count.zero?
      end
  ...
  def self.add_new_instance_for_cud_grace_day_usage(course, condition_id, cud, asmts_before_date, grace_day_threshold)
    ...
    auds_before_date_filtered = auds_before_date.compact
    ...
    if latest_aud_before_date.global_cumulative_grace_days_used >= grace_day_threshold
      ...
      auds_before_date.each do |aud|
        if aud.grace_days_used.positive?
          violation_info[aud.assessment.display_name] =
            aud.grace_days_used
        end
      end
  ...
  def self.add_new_instance_for_cud_grade_drop(course, condition_id, cud, asmt_arrs, consecutive_counts, percentage_drop)
    ...
    asmt_arrs.each do |asmts|
      ...
      violation_info[asmts[0].category_name] = violating_pairs unless violating_pairs.empty?
    end

    unless violation_info.empty?
      WatchlistInstance.new(course_user_datum_id: cud.id, course_id: course.id,
                            risk_condition_id: condition_id,
                            violation_info: violation_info)
