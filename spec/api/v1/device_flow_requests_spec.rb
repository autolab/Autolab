...
RSpec.describe Oauth::DeviceFlowController, :type => :controller do
  describe 'GET #init' do
    ...
    it 'returns a valid device_code and user_code pair' do
      get :init, :client_id => df_application.uid
    ...
    it 'does not allow invalid client_id' do
      get :init, :client_id => df_application.uid.length.times.map { (65 + rand(26)).chr }.join
  ...
  describe 'GET #authorize' do
    ...
    context 'when given the right params' do
      ...
      it 'returns pending when not resolved' do
        get :authorize, :client_id => df_application.uid, :device_code => @req.device_code
        expect(response.response_code).to eq(400)
      ...
      it 'returns denied when access denied' do
        ...
        get :authorize, :client_id => df_application.uid, :device_code => device_code
        expect(response.response_code).to eq(400)
    ...
    it 'does not allow invalid device_code' do
      get :authorize, :client_id => df_application.uid, :device_code => 32.times.map { (65 + rand(26)).chr }.join
      expect(response.response_code).to eq(400)
    end

    it 'does not allow missing device_code' do
      get :authorize, :client_id => df_application.uid
      expect(response.response_code).to eq(400)
    end

    it 'does not allow invalid client_id' do
      get :authorize, :client_id => df_application.uid.length.times.map { (65 + rand(26)).chr }.join
      expect(response.response_code).to eq(400)
    end
