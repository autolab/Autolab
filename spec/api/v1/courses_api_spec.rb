...
RSpec.describe Api::V1::CoursesController, :type => :controller do
  ...
  describe 'GET create' do
    ...
    it 'fails to authenticate when the app does not have admin scope' do
      get :create, :access_token => token.token
      expect(response.response_code).to eq(403)
    end

    it 'fails to authenticate when the user is not an admin' do
      get :create, :access_token => admin_token_for_user.token
      expect(response.response_code).to eq(403)
    end

    it 'fails to create when name is invalid' do
      get :create, :access_token => admin_token_for_admin.token, :name => "Hello There", :semester => @course_sem, :instructor_email => @instructor_email
      expect(response.response_code).not_to eq(200)
    end

    it 'creates a course successfully for an existing user' do
      get :create, :access_token => admin_token_for_admin.token, :name => @course_name, :semester => @course_sem, :instructor_email => @instructor_email
      expect(response.response_code).to eq(200)
    ...
    it 'creates a course successfully for a non-existing user' do
      ...
      get :create, :access_token => admin_token_for_admin.token, :name => @course_name, :semester => @course_sem, :instructor_email => new_email
      expect(response.response_code).to eq(200)
