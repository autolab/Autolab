...
RSpec.describe Api::V1::CoursesController, type: :controller do
  describe "GET #index" do
    ...
    it "fails to authenticate when the token is invalid" do
      get :index, params: { access_token: token.token.length.times.map do
                                            rand(65..90).chr
                                          end.join } # a random token
  ...
  describe "GET create" do
    ...
    it "fails to create when name is invalid" do
      get :create,
          params: { access_token: admin_token_for_admin.token, name: "Hello There", semester: @course_sem,
    ...
    it "creates a course successfully for an existing user" do
      get :create,
          params: { access_token: admin_token_for_admin.token, name: @course_name, semester: @course_sem,
                    instructor_email: @instructor_email }
      expect(response.response_code).to eq(200)
