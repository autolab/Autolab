...
module Rack
  class Attack
    ...
    safelist("allow from localhost") do |req|
      (!req.path.start_with?("/api/") || !req.params["access_token"]) &&
        !req.path.start_with?("/oauth/device_flow_")
    end
    ...
    throttle("api/general", limit: 10, period: 30.seconds) do |req|
      req.user_id if req.path.start_with?("/api/")
    end
    ...
    throttle("api/submit", limit: 4, period: 1.minute) do |req|
      req.user_id if req.path.end_with?("submit")
    end
    ...
    throttle("oauth/device_flow_init", limit: 1, period: 30.seconds) do |req|
      req.ip if req.path.start_with?("/oauth/device_flow_init")
    end
    ...
    throttle("oauth/device_flow_authorize", limit: 1, period: 5.seconds) do |req|
      req.ip if req.path.start_with?("/oauth/device_flow_authorize")
    end
