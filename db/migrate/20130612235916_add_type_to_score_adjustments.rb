...
class AddTypeToScoreAdjustments < ActiveRecord::Migration[4.2]
  def self.up
    ...
    Course.find_each do |c|
      set_type c.late_penalty_id, "Penalty"
      set_type c.version_penalty_id, "Penalty"
    end

    Assessment.find_each do |a|
      set_type a.late_penalty_id, "Penalty"
      set_type a.version_penalty_id, "Penalty"
    end

    User.find_each do |u|
      set_type u.tweak_id, "Tweak"
    end

    Submission.find_each do |s|
      set_type s.tweak_id, "Tweak"
    end
  end
