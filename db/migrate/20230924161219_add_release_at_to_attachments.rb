  ...
  def up
    ...
    Attachment.find_each do |attachment|
      if attachment.released
        attachment.update_attribute(:release_at, DateTime.now)
      else
        attachment.update_attribute(:release_at, DateTime.now + 1.year)
      end
    end
  end
  def down
    Attachment.find_each do |attachment|
      attachment.update_attribute(:released, attachment.release_at < DateTime.now)
    end
