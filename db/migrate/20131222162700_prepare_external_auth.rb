...
class PrepareExternalAuth < ActiveRecord::Migration[4.2]
  def up
    ...
    if table_exists? :authentications_backup
    ...
    if column_exists? :course_user_data, :user_id_backup
    ...
    CourseUserDatum.find_each do |cud|
      if cud.user.nil? # no user field
        user = User.where(email: cud.email_backup).first

        if user.nil? # user haven't been created yet
    ...
    if index_exists?(:assessment_user_data, %w[user_id assessment_id])
      remove_index :assessment_user_data, %w[user_id assessment_id]
    end
    rename_column :assessment_user_data, :user_id,
                  :course_user_datum_id
    add_index :assessment_user_data, %w[course_user_datum_id assessment_id],
  ...
  def down
    ...
    rename_column :assessment_user_data, :course_user_datum_id,
                  :user_id
    add_index "assessment_user_data", %w[user_id assessment_id]
